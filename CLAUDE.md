...
- `models.py` — SQLite schema (`tasks` table), Pydantic models (`TaskSubmit`, `TaskStatus`, `GpuStatus`), DB helper functions. DB write helpers (`set_task_running`, `set_task_finished`, `set_task_aborted`) all return `bool` and include state-transition guards (e.g. `WHERE state = 'PENDING'`) to prevent races between dispatch and abort. On `init_db`, any tasks stuck in `RUNNING` state from a prior crash are moved to `FAILED` (exit_code=-1).
- `scheduler.py` — `Scheduler` is the async orchestrator. `_run_loop` calls `_dispatch_pending_tasks` (sorted by priority DESC, created_at ASC) then `_check_running_tasks`. Tasks spawn via `asyncio.create_subprocess_shell` with `preexec_fn=os.setsid` (process group for clean kill). Commands are wrapped with `sudo -u <user> bash -l -c` for per-user execution, and `CUDA_VISIBLE_DEVICES=<gpu_id>` for GPU isolation. Conda envs are resolved by locating the env's python binary in `/home/<user>/<conda_dir>/envs/<name>/bin/python` and replacing `python`/`python3` in the command string via regex. `_spawn_task` checks `set_task_running` return value — if an abort sneaked in mid-dispatch, it kills the spawned process and cleans up.
- `main.py` — FastAPI app with lifespan-managed globals (`db`, `gpu_manager`, `scheduler`). REST endpoints: task submit/status/abort/log, GPU telemetry, user listing (parsed from `/etc/passwd`), per-user conda env scanning, per-user workdir browsing with path traversal guard. Static files (`static/index.html`) mounted after all API routes.

**Frontend:** Single-page dashboard in `static/index.html` — vanilla JS + Tailwind CSS CDN. Polls `/health`, `/gpus`, `/tasks/status` every 3s. Includes task submission form (with user selector, GPU selector, conda env picker, workdir browser modal), GPU cards, filterable task table, log viewer modal (works for both running and completed tasks), and directory browser modal.
